...
public struct ZenBookmarkImporter {
    ...
    private static var zenRoot: URL {
        let base = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
        return base.appendingPathComponent("zen")
    }

    private static func locatePlacesSQLite() -> URL? {
        let profilesDir = zenRoot.appendingPathComponent("Profiles")
    ...
    private static func copyDatabase(from source: URL) -> URL? {
        let tempDir = FileManager.default.temporaryDirectory
            .appendingPathComponent("sift-zen-\(UUID().uuidString)")
        do {
