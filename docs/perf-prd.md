...
### [ ] 1. Gate `DebugLog.write` so it's a no-op outside debug builds
...
### [ ] 2. Debounce `SiftViewModel.updateQuery` by ~40 ms
...
### [ ] 3. Precompute `searchableApps` instead of filtering per keystroke
...
### [ ] 4. Memoize `highlightedName` per result instead of rebuilding `AttributedString` on every render
...
### [ ] 5. Cap audio meter to ~20 fps and use a ring buffer instead of reallocating `bars`
...
### [ ] 6. Cache `AudioService.isRunning` inside `AudioMeterService` and read from there in the status strip
...
### [ ] 7. Cache `NSRunningApplication` lookups by PID in `refreshSources`
...
### [ ] 8. Reuse the backdrop window across panel opens
...
### [ ] 9. `FaviconCache.pendingHosts` → `Set<String>` for O(1) dedup
...
### [ ] 10. Move `AppIndex.scan` off the main thread in `SettingsViewModel.init`
...
### [ ] 11. Persist `AppIconCache` to disk so cold launches don't re-fetch
...
### [ ] 12. Lower `PsychedelicView` redraw rate when intensity is low
...
### [ ] 13. `NowPlayingService.refresh` — only invoke `getInfo` when the playback state changes
...
### [ ] 14. Stop reassigning `SearchField` closures every `updateNSView`
